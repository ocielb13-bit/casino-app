...
function normalizeSettings(input = {}) {
  ...
  merged.win_rate = clamp(Number(merged.win_rate), 1, 99.9);
  merged.multiplier = clamp(Number(merged.multiplier), 1, 100);
  merged.jackpot_bank = Math.max(1000, Math.floor(Number(merged.jackpot_bank)));
  merged.default_balance = Math.max(0, Math.floor(Number(merged.default_balance)));
  merged.slot_pay_3 = Math.max(0, Math.floor(Number(merged.slot_pay_3)));
  merged.slot_pay_4 = Math.max(0, Math.floor(Number(merged.slot_pay_4)));
  merged.slot_pay_5 = Math.max(0, Math.floor(Number(merged.slot_pay_5)));
  merged.roulette_payout = Math.max(1, Math.floor(Number(merged.roulette_payout)));
  merged.free_spin_award = Math.max(0, Math.floor(Number(merged.free_spin_award)));
...
async function getUserById(id) {
  ...
  const user = {
    ...
    free_spins: clampInt(data.free_spins, 0, 20),
    free_spin_bank: clampInt(data.free_spin_bank, 0, 20)
  };

  if (
    user.free_spins !== Number(data.free_spins || 0) ||
    user.free_spin_bank !== Number(data.free_spin_bank || 0)
  ) {
...
async function getLoginUserByUsername(username) {
  const { data, error } = await supabase
    .from("app_users")
    .select("id, username, role, balance, free_spins, free_spin_bank, password")
...
function pickSymbol({ allowScatter = true, exclude = [] } = {}) {
  ...
  const safePool = pool.length
    ? pool
    : SYMBOL_WEIGHTS.filter((item) => !exclude.includes(item.symbol));

  return weightedPick(safePool);
}
...
function buildBoard(settings) {
  ...
  if (roll < winRate * 0.85) return buildWinningBoard(4);
  if (roll < winRate) return buildWinningBoard(5);
  if (roll < winRate + 10) return buildNearMissBoard();
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    await saveUserState(normalizedUser.id, {
      free_spins: normalizedUser.free_spins,
      free_spin_bank: normalizedUser.free_spin_bank
    }).catch(() => {});
...
app.get("/api/me", authRequired, (req, res) => {
  res.json({
    ...
    free_spins: clampInt(req.user.free_spins, 0, 20),
    free_spin_bank: clampInt(req.user.free_spin_bank, 0, 20)
  });
});
...
app.get("/api/admin/users", authRequired, adminOnly, async (req, res) => {
  ...
  const users = (data || []).map((u) => ({
    ...
    free_spins: clampInt(u.free_spins, 0, 20),
    free_spin_bank: clampInt(u.free_spin_bank, 0, 20)
  }));
...
app.post("/api/slots/spin", authRequired, async (req, res) => {
  try {
    ...
    let nextFreeSpins = clampInt(user.free_spins, 0, 20);
    let nextFreeBank = clampInt(user.free_spin_bank, 0, 20);
    ...
    const noFreeLeft = nextFreeSpins + nextFreeBank === 0;

    let freeSpinsAwarded = 0;
    let nextJackpotBank = Number(jackpotBank || DEFAULT_SETTINGS.jackpot_bank) + Math.max(1, Math.floor(bet * 0.02));

    if (outcome.jackpotHit) {
      nextJackpotBank = DEFAULT_SETTINGS.jackpot_bank;
    }

    if (outcome.scatterCount >= 3 && noFreeLeft) {
      if (Math.random() < 0.20) {
